...
\code{timeit} module method \code{timeit} to time a single statement (denoted by
\code{stmt}). The statement is repeated \code{n} times to get the average time per call.
...
timer calls. How do the times here, using \code{numpy}, compare with the times for single
statments using \code{timeit}?

Now use the \code{timeit.default\_timer()} calls to time the loop over random number
