...
In function \code{back\_recursion}, we first initialize an empty vector \code{I} which
will store the values of the modified Bessel functions of each order up to
\code{max\_order} ($30$ in this example) at $x$, i.e., $I_n(x)$, where
$n = 1, 2, \ldots, 30$.
Then we set the last two values of \code{I} to be anything we want, $1$ and $0$ here,
